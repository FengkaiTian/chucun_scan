...
结果文件已存在即拒绝重跑。判据见 PREREGISTRATION_timing.md。主检验先算；附加参考失败不影响主检验。"""
...
try:
    ...
    ref = (f"{r.index[0]:%Y-%m-%d} ~ {r.index[-1]:%Y-%m-%d}，{old.attrs['source']}；平均仓位 {w.mean():.2f}\n"
           f"| 指标 | 值 |\n|---|---|\n{tbl(n)}\n\n| 判据 | 结果 |\n|---|---|\n{verdict(k)}\n")
except Exception as e:
    ref = f'附加参考计算失败（不影响主检验）：{type(e).__name__}: {e}\n'
